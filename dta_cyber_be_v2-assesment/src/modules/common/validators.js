            ...
            Object.keys(this.fields).map(async field => {
                ...
                if (!isNull(functionValidator) && isNull(this.errors[field])) {
                    try {
                        ...
                        const returnedValue = await functionValidator(fieldValue, this.data)
...
/**
 * Given sequelize model and the filter or pk. This will return the instance.
 * If the instance is not preset, this will throw an exception.
 */
const getInstanceOrException = async (model, pk = null, filter = null) => {
    ...
    throw new ApiError('Invalid args passed.')
}
